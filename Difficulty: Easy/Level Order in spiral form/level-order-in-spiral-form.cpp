...
class Solution {
  public:
    vector<int> findSpiral(Node* root) {
        stack<Node*>leftFirst, rightFirst;
        rightFirst.push(root);
        vector<int>ans;
        while(!leftFirst.empty() || !rightFirst.empty()){
            while(!leftFirst.empty()){
                
                Node* node=leftFirst.top();
                leftFirst.pop();
                ans.push_back(node->data);
                if(node->left) rightFirst.push(node->left);
                if(node->right) rightFirst.push(node->right);
                
            }
            
            
            while(!rightFirst.empty()){
                Node* node=rightFirst.top();
                rightFirst.pop();
                ans.push_back(node->data);
                if(node->right) leftFirst.push(node->right);
                if(node->left) leftFirst.push(node->left);
            }
        }
        return ans;
